...
export default Ember.View.extend({
  ...
  didInsertElement: function() {
    ...
    Ember.$(dragTrigger).appendTo($view);
  ...
  touchStart: function(event) {
    ...
    var layer = Ember.$(touchEvent.target).closest('.mobile-drag-trigger')[0];
    if (layer) {
      this.active = Ember.$(layer).parents('.pane')[0];
      this.onStart(event, touchEvent);
      this.activeWidth = Ember.$(this.active).outerWidth();
    }
  },
  ...
  actions: {
    closePropertyWithTransition: function() {
